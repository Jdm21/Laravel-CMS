...
class NewsController extends Controller
{
    ...
    public function store(NewsRequest $request)
    {
        ...
        if(!empty($file)) {
            ...
            $ImageManager = Image::make(storage_path('app/news/') . $data['cover']);
            $ImageManager->resize(300, 200);
            $ImageManager->save(storage_path('app/news/') . 'thumbnails/' . $data['cover']);
        }
    ...
    public function update(NewsRequest $request, News $news)
    {
        ...
        session()->flash("success", "O registro foi editado com sucesso!");
